...
static struct mtk_panel *poweroff_ctx = NULL;
...
#define A16_HBM_BRIGHTNESS 460
#define A16_MAX_BRIGHTNESS 255
#define A16_MIN_BRIGHTNESS 10

static int mcd_drm_parse_dt(void *_ctx, struct device_node *np)
{
	int ret = 0;
	struct mtk_panel *ctx = (struct mtk_panel *)_ctx;
	pr_err("%s start, lcm_id=%d\n", __func__, lcm_id);
	...
	if(1 == lcm_id)
		ctx->spec.vdo_per_frame_lp_enable = 1;
...
{
	printk("%s, + fanny\n", __func__);
	mcd_panel_check_probe(poweroff_ctx);

        mcd_panel_set_power(poweroff_ctx, 0);
	printk("%s, -\n", __func__);
}
EXPORT_SYMBOL(mcd_panel_poweroff);
...
static int sand_brightness_cust(int32_t brightness, int sensor_type,
		int action)
{
	...
	cmd.tx_len = 8;
	cmd.rx_len = 0;
	cmd.data[0] = brightness;
	pr_info("brightness = %d\n", cmd.data[0]);
	hf_client_custom_cmd(client, sensor_type, &cmd);
out:
...
{
	unsigned int mapping_level = 0x03ff;
	...
	char global_para2[] = { 0xb0, 0x00, 0xF8, 0xB5};
	char IRC_mode[] = { 0xB5, 0x01, 0x00, 0x00, 0x00, 0x86, 0xE2, 0x44, 0x80, 0x00, 0x00, 0xFF, 0xF4, 0xFF, 0x9C };//flat mode
	unsigned int flat_z_mode = 0x82;

	if(level >= A16_HBM_BRIGHTNESS ){
		level = A16_HBM_BRIGHTNESS;
		IRC_mode[1] = flat_z_mode;
	}

	if(level <= A16_MIN_BRIGHTNESS)
	{
		mapping_level = 0x0004;
	}
	else if (level <= A16_MAX_BRIGHTNESS)
	{
		mapping_level = (level-A16_MIN_BRIGHTNESS) * 2047 / (A16_MAX_BRIGHTNESS-A16_MIN_BRIGHTNESS);
	}
	else
	{
		mapping_level = (level-A16_MIN_BRIGHTNESS) * 4095 / (A16_HBM_BRIGHTNESS-A16_MIN_BRIGHTNESS);
	}

	bl_tb0[1] = ((mapping_level >> 8) & 0xf);
	bl_tb0[2] = (mapping_level & 0xff);

	pr_info("[%s][mcd_panel_adapter] bl_tb0[0] = 0x%x, bl_tb0[1] = 0x%x, level = %d(0x%x)\n",
	...
	cb(dsi, handle, global_para2, ARRAY_SIZE(global_para2));
	cb(dsi, handle, IRC_mode, ARRAY_SIZE(IRC_mode));
	...
	sand_brightness_cust((int32_t)level, SENSOR_TYPE_LIGHT, CUST_CMD_TEST);
	return 0;
}
...
mcd_panel_get_mtk_panel_desc(struct mtk_panel *ctx)
{
	...
	if (pdms[0].modes[0]->panel_video_mode)
		dsi->mode_flags |= (MIPI_DSI_MODE_VIDEO | MIPI_DSI_MODE_VIDEO_SYNC_PULSE /*| MIPI_DSI_MODE_VIDEO_BURST*/); /* todo LPM? */
